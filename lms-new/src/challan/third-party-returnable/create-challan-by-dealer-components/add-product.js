...
function AddProduct({ addProductModal, setAddProductModal, api }) {
  ...
  const getProductsOnSite = async (productType) => {
    ...
    try {
      const { data } = await axios.post(
        window.MyApiRoute +
        `record/get?location=onSiteAndInstalledByDealer&category=${productType}`,
        { ...a }
      );
      // console.log(data);
  ...
  const handleSubmit = async () => {
    ...
    if (addProductModal.type === "newProduct") {
      try {
        setLoading(true)
        await axios.post(
          window.MyApiRoute +
          `record/add?location=forDealer&category=${input.productType}`,
          { ...a, ...input }
        );
      } catch (error) {
        setLoading(true)
      ...
      try {
        ...
        await axios.put(
          window.MyApiRoute +
          `record/update?check=TempThirdPartyQueByDealer&category=${input.productType}`,
          { ...a, ...input }
        );
      } catch (error) {
        setLoading(true)
