...
const EngineerChallan = () => {
  ...
  const handleOptionChange = (option) => {
    setSelectedOption(option);
    navigate(`${mainRoute}/challan/engineer-returnable/${option}`, { replace: true });
  };

  useEffect(() => {
    // Extract the challan type from the current pathname
    const pathSegments = pathname.split("/");
    const currentChallanType = pathSegments[4]; // Adjusted for /erp/challan/engineer-returnable/<type>
    if (currentChallanType === undefined) {
      navigate(`${mainRoute}/challan/engineer-returnable/${selectedOption}`, {
        replace: true,
      });
      return;
    }
