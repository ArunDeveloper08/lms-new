...
const SendToCRMModal = ({
    ...
    axios
      ...
      .then((res) => {
        ...
        const engineerData = res.data?.data.filter(
          (employee) => employee?.Designation === "CRM"
        );
  ...
  const handleSubmit = () => {
    axios
      .put(`${window.MyApiRoute}tool/update?location=CRM`, {
        ...moreDetails,
        ToolID: tool?.id,
  ...
  return (
    <div>
      <Modal open={open3} onClose={() => setOpen3(!open3)}>
        <Box
          sx={{
            position: "absolute",
            top: "50%",
          ...
          {userInfo.Designation === "storekeeper" && (
            <div className="flex justify-center mt-[10px]">
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Select CRM Name" />
                )}
              />
            </div>
          )}
