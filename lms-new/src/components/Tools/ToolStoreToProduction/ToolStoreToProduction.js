...
const ToolStoreToProduction = ({ canChange, query, downloadUrl }) => {
  ...
  const api = () => {
    axios
      .post(
        `${window.MyApiRoute}tool/get?location=storeToProduction&Employee_Id=${userInfo?.data.Employee_Id}&ToolID=${tool?.id}`,
        userInfo.data
      )
      .then((res) => {
...
export default ToolStoreToProduction;
