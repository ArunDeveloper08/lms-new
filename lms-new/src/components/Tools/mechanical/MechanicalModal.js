...
const  MechanicalModal = ({ open2, setOpen2, checked, tool ,  setBadgeCount ,setOpens ,setChecked }) => {
    ...
    axios
      ...
      .then((res) => {
        const engineerData = res.data?.data.filter(
          (employee) => employee?.Designation === "Mechanical"
        );
  ...
  const handleSubmits = () => {
    axios
      .put(`${window.MyApiRoute}tool/update?location=mechanical`, {
        ...moreDetails,
        ToolID:tool?.id,
       ...userInfo,
      })
  ...
  return (
    <div>
      <Modal open={open2} onClose={() => setOpen2(!open2)}>
        <Box
          sx={{
            position: "absolute",
            top: "50%",
