...
const AddProductEngineerModal = ({ modal, setModal, api }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.put(
        `${window.MyApiRoute}tool/upadteTempTool?check=createEngineerTemp`,
        {
          ...a.data,
          ...values,
        }
    ...
    async function getProductList() {
      ...
      try {
        ...
        const res = await axios.post(
          `${window.MyApiRoute}tool/getToolForChallan?toolName=${values.productType}`,
          {
            ...a.data,
          }
        );
    ...
    >
      ...
      <DialogContent className="space-y-4 flex flex-col w-[500px]">
        ...
        <FormControl className="flex-1">
          ...
          <Select
            className="rounded h-[50px] border-gray-700 border-[1px] ml-2 mb-2"
            value={values.productType} // Ensure the selected value is shown
            onChange={(e) => onChange("productType", e.target.value)} // ✅ Correctly update state
            name="productType"
            label="Product Type"
          >
            {tools.map((item, index) => (
              <MenuItem key={index} value={item?.ToolName}>
                {item?.ToolName}
              </MenuItem>
            ))}
          </Select>
        ...
        {/* when the product is already on engineer  */}
        {productList.loading ? (
          <p className="text-center">Loading...</p>
        ) : productList.error ? (
...
export default AddProductEngineerModal;
